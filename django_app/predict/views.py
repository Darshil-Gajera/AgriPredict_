...
def save_result_view(request):
    try:
        ...
        SavedResult.objects.create(
            ...
            theory_marks=float(data.get('theory') or data.get('theory_obtained', 0)),
            theory_total=int(data.get('theory_total', 300)),
            gujcet_marks=float(data.get('gujcet') or data.get('gujcet_marks', 0)),
            student_category=data.get('student_category', 'OPEN'),
            farming_bonus=bool(data.get('farming') or data.get('farming_background', False)),
        )
